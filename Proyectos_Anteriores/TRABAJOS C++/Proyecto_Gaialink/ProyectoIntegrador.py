...
import webbrowser
#================================ADMIN CREDENCIALES================================================================================================================
correo_emisor = "gaialink.contacto@gmail.com"
contraseña = "gfup sgov wwai rjuc"
correo_receptor1 = "gaialink.directivas@gmail.com"
fuente = "Lucida Fax"
Contador = 0
Max_Intentos = 3
...
Incidentes_Valores = {
    ...
    "Hurto": "Hurt",
    "Expropiacion": "Expro",
    "Predios-Despojados": "Despo"
}
Incidentes_Valores2 = {
    "Despl": "Desplazamiento",
    "Hurt": "Hurto",
    "Expro": "Expropiacion",
    "Despo": "Predios-Despojados"
}
...
root = CTk()
root.title("GaiaLink")
root.configure(fg_color="#1b1b1b")
...
#============IMAGENES============
imagen_original_fondo = Image.open("Fondo.png")
lblimagen = Image.open("Logo Gaialink.png")
...
Imagen_Eliminar = Image.open("Eliminar.png")
...
def actualizar_fondo(event=None):
    ...
    if (width, height) != tuple(last_size):
        ...
        nueva_imagen = imagen_original_fondo.resize((width, height), Image.Resampling.LANCZOS)
        imagen_tk = ImageTk.PhotoImage(nueva_imagen)
        fondo_label.config(image=imagen_tk)
...
class Caso:
    def __init__(self, TipoIncidente, Fecha, Descripcion, Direccion, Afectados):
...
def Funcion_Crear_Caso():
     Conexion_Base_De_Datos()
     TipoIncidente = Entrada_Caso_TipoIncidente.get()
     ...
     if not all([TipoIncidente, Fecha, Descripcion, Direccion, Afectados]):
        messagebox.showerror("Error", "Todos los campos son obligatorios")
        Cerrar_Base_De_Datos()
        return
     caso = Caso(TipoIncidente, Fecha, Descripcion, Direccion, Afectados)
     ...
     id_caso = "Caso"
     if TipoIncidente == "Desplazamiento":
         id_Incidente = "Despl"
     elif TipoIncidente == "Hurto":
         id_Incidente = "Hurt"
     elif TipoIncidente == "Expropiacion":
         id_Incidente = "Expro"
     elif TipoIncidente == "Predios-Despojados":
         id_Incidente = "Despo"
     ...
     try:
        ...
        cursor.execute("INSERT INTO tbl_caso (Id_Caso_Incidente, Fecha, Descripción, Personas_Afectadas, Fk_Usuario, Fk_Incidente, Fk_Dep, Fk_Tipo_Caso, Fk_Estado) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)", 
                       (id_Caso_Incidente, caso.Fecha, caso.Descripcion, caso.Afectados, id_usuario, id_Incidente, id_departamento, id_caso, id_estado))

        cursor.execute("INSERT INTO tbl_num_caso (Id_num_caso, Radicado, Fk_Caso) VALUES (%s, %s, %s)", 
                       (id_entidad, radicado, id_Caso_Incidente)) 
...
def Funcion_Buscar_Caso2():
    global Radicado, Radicado_Fecha, Radicado_Descripcion, Radicado_Personas, Radicado_Usuario, Radicado_Incidente, Radicado_Departamento, Radicado_Tipo, Radicado_Estado
    Conexion_Base_De_Datos()
    ...
    try:
        ...
        cursor.execute("SELECT tbl_caso.Fk_Estado FROM tbl_num_caso JOIN tbl_caso ON tbl_num_caso.Fk_Caso = tbl_caso.Id_Caso_Incidente WHERE tbl_num_caso.Radicado = %s", (Radicado,))
        resultado_estado = cursor.fetchone()
        if not resultado_estado:
        ...
        query = """
            ...
            JOIN tbl_Incidente ON tbl_caso.Fk_Incidente = tbl_Incidente.Id_Incidente
            ...
            JOIN tbl_num_caso ON tbl_caso.Id_Caso_Incidente = tbl_num_caso.Fk_Caso
        ...
        if not resultado:
            ...
            return
        fecha, descripcion, personas, usuario, Incidente, departamento, tipo, estado = resultado
...
def Funcion_Buscar_Entidades():
    try:
        ...
        ventana.config(background="#1b1b1b")
        ventana.lift()
        ventana.focus_force()
        ventana.grab_set()
        ancho, alto = 1200, 580
        ...
        barra_superior = CTkFrame(ventana, height=40, fg_color="#1b1b1b", border_color="#1b1b1b")
        barra_superior.pack(fill="x", padx=20, pady=(10, 0))

        Boton_Salir = CTkButton(barra_superior, image=CTkImage(dark_image=Imagen_Boton_Salir, size=(45, 45)), hover_color="#1b1b1b", text="", width=45, height=45, bg_color="#1b1b1b", fg_color="#1b1b1b", command=ventana.destroy)
        ...
        columnas = (
            "ID Entidad", "Nombre", "Descripción Ent", "Incidente",
...
def Funcion_Cargar_Datos_Casos():
    global Estado_Original, Fecha_Formateada, Fecha_Original, Descripcion_Original, Afectados_Original, Incidente_Original, Departamento_Original, Tipo_Original, Incidente_Convertido, Estado_Convertido, Departamento_Convertido
    Conexion_Base_De_Datos()
    cursor.execute("SELECT tbl_caso.Fk_Estado FROM tbl_num_caso JOIN tbl_caso ON tbl_num_caso.Fk_Caso = tbl_caso.Id_Caso_Incidente WHERE tbl_num_caso.Radicado = %s", (Radicado,))
    resultado_estado = cursor.fetchone()
    if not resultado_estado:
    ...
    if Estado_Actual == "Caso_03":
        messagebox.showerror("Error", "El numero de radicado no existe.")
        return
    cursor.execute("SELECT tbl_caso.Fk_Estado, tbl_caso.Fecha, tbl_caso.Descripción, tbl_caso.Personas_Afectadas, tbl_caso.Fk_Incidente, tbl_caso.Fk_Dep, tbl_caso.Fk_Tipo_Caso FROM tbl_caso JOIN tbl_num_caso ON tbl_caso.Id_Caso_Incidente = tbl_num_caso.Fk_Caso WHERE tbl_num_caso.Radicado = %s", (Radicado,))
    datos = cursor.fetchone()
    if not datos:
        messagebox.showerror("Datos no encontrados", "Los datos no existen.")
        return
    Estado_Original, Fecha_Original, Descripcion_Original, Afectados_Original, Incidente_Original, Departamento_Original, Tipo_Original = datos
    Incidente_Convertido = Incidentes_Valores2.get(Incidente_Original)
...
def Funcion_Modificar_Datos_Casos():
    ...
    Incidente_Mitad = Entrada_Caso_Incidente.get()
    Incidente_Listo = Incidentes_Valores.get(Incidente_Mitad)
    ...
    try:
        ...
        cursor.execute("UPDATE tbl_caso JOIN tbl_num_caso ON tbl_caso.Id_Caso_Incidente = tbl_num_caso.Fk_Caso SET Fk_Estado = %s WHERE Radicado = %s", (Estado_Listo, Radicado))
        cursor.execute("UPDATE tbl_caso JOIN tbl_num_caso ON tbl_caso.Id_Caso_Incidente = tbl_num_caso.Fk_Caso SET Fecha = %s WHERE Radicado = %s", (Fecha_Listo, Radicado))
        cursor.execute("UPDATE tbl_caso JOIN tbl_num_caso ON tbl_caso.Id_Caso_Incidente = tbl_num_caso.Fk_Caso SET Descripción = %s WHERE Radicado = %s", (Descripcion_Listo, Radicado))
        cursor.execute("UPDATE tbl_caso JOIN tbl_num_caso ON tbl_caso.Id_Caso_Incidente = tbl_num_caso.Fk_Caso SET Personas_Afectadas = %s WHERE Radicado = %s", (Afectados_Listo, Radicado))
        cursor.execute("UPDATE tbl_caso JOIN tbl_num_caso ON tbl_caso.Id_Caso_Incidente = tbl_num_caso.Fk_Caso SET Fk_Incidente = %s WHERE Radicado = %s", (Incidente_Listo, Radicado))
        cursor.execute("UPDATE tbl_caso JOIN tbl_num_caso ON tbl_caso.Id_Caso_Incidente = tbl_num_caso.Fk_Caso SET Fk_Dep = %s WHERE Radicado = %s", (Departamento_Listo, Radicado))
        cursor.execute("UPDATE tbl_caso JOIN tbl_num_caso ON tbl_caso.Id_Caso_Incidente = tbl_num_caso.Fk_Caso SET Fk_Tipo_Caso = %s WHERE Radicado = %s", (Tipo_Listo, Radicado))
        ...
        Entrada_Caso_Incidente.place_forget()
        ...
        Entrada_Caso_Incidente_Check.place_forget()
...
def Funcion_Eliminar_Datos_Casos():
    ...
    try:
        ...
        cursor.execute("UPDATE tbl_caso JOIN tbl_num_caso ON tbl_caso.Id_Caso_Incidente = tbl_num_caso.Fk_Caso SET Fk_Estado = %s WHERE Radicado = %s", (Estado, Radicado))
...
# Botones generales y usuario
    Boton_Salir.place_forget()
    Boton_Usuario_Cuenta_Volver.place_forget()
...
# Entradas tipo Text y Combobox específicos
    ...
    Entrada_Caso_Fecha.place_forget()
    Entrada_Caso_TipoIncidente.place_forget()
...
# Limpieza de campos (delete y set)
    ...
    Entrada_Caso_TipoIncidente.set("Seleccion")
def Grafico_Inicio():
    Olvidar_Lugares()
    ...
    Texto1.configure(text="GAIA", text_color="black", font=(fuente, 60))
    Texto2.configure(text="LINK", text_color="black", font=(fuente, 60))
    Texto1.place(relx=0.5, rely=0.18, anchor="center")
    Texto2.place(relx=0.5, rely=0.30, anchor="center")
def abrir_whatsapp():
    numero = "14155238886"
    mensaje = "Hola"
...
def Grafico_Pagina():
    ...
    root3.config(background="#1b1b1b")
    root3.resizable(height = True, width = True)
    root3.lift()
    root3.focus_force()
    root3.grab_set()
    Boton_Salir3 = CTkButton(root3, image=CTkImage(dark_image=Imagen_Boton_Salir, size=(45, 45)), hover_color="#1b1b1b", text="", width=45, height=45, bg_color="#1b1b1b", fg_color="#1b1b1b", command=root3.destroy)
    Boton_Salir3.place(relx=0.015, rely=0.015, anchor="nw")

    Texto38 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text="Usuario", font=(fuente, 45))
    Texto1 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text=f"{Usuario}", font=(fuente, 45))
    Texto2 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text="ID", font=(fuente, 17))
    Texto3 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text="Tipo ID", font=(fuente, 17))
    Texto4 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text="Primer Nombre", font=(fuente, 17))
    Texto5 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text="Segundo Nombre", font=(fuente, 17))
    Texto6 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text="Primer Apellido", font=(fuente, 17))
    Texto7 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text="Segundo Apellido", font=(fuente, 17))
    Texto8 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text="Usuario", font=(fuente, 17))
    Texto9 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text="Rol", font=(fuente, 17))
    Texto10 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text="Estado", font=(fuente, 17))
    Texto11 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text="Departamento", font=(fuente, 17))
    Texto12 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text="Ciudad", font=(fuente, 17))
    Texto13 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text="Localidad", font=(fuente, 17))
    Texto14 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text="Barrio", font=(fuente, 17))
    Texto15 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text="Direccion", font=(fuente, 17))
    Texto16 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text="Fecha Nacimiento", font=(fuente, 17))
    Texto17 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text="Edad", font=(fuente, 17))
    Texto18 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text="Correo", font=(fuente, 17))
    Texto19 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text="Telefono", font=(fuente, 17))

    Texto20 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text=f"{Busqueda2_Documento}", font=(fuente, 18, "bold"))
    Texto21 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text=f"{Busqueda2_TipoDocumento}", font=(fuente, 18, "bold"))
    Texto22 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text=f"{Busqueda2_PrimerNombre}", font=(fuente, 18, "bold"))
    Texto23 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text=f"{Busqueda2_SegundoNombre}", font=(fuente, 18, "bold"))
    Texto24 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text=f"{Busqueda2_PrimerApellido}", font=(fuente, 18, "bold"))
    Texto25 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text=f"{Busqueda2_SegundoApellido}", font=(fuente, 18, "bold"))
    Texto26 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text=f"{Busqueda2_NombreUsuario}", font=(fuente, 18, "bold"))
    Texto27 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text=f"{Busqueda2_RolUsuario}", font=(fuente, 18, "bold"))
    Texto28 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text=f"{Busqueda2_EstadoUsuario}", font=(fuente, 18, "bold"))
    Texto29 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text=f"{Busqueda2_Departamento}", font=(fuente, 18, "bold"))
    Texto30 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text=f"{Busqueda2_Ciudad}", font=(fuente, 18, "bold"))
    Texto31 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text=f"{Busqueda2_Localidad}", font=(fuente, 18, "bold"))
    Texto32 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text=f"{Busqueda2_Barrio}", font=(fuente, 18, "bold"))
    Texto33 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text=f"{Busqueda2_Direccion}", font=(fuente, 18, "bold"))
    Texto34 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text=f"{Busqueda2_FechaNacimento}", font=(fuente, 18, "bold"))
    Texto35 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text=f"{Busqueda2_Edad}", font=(fuente, 18, "bold"))
    Texto36 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text=f"{Busqueda2_Email}", font=(fuente, 18, "bold"))
    Texto37 = CTkLabel(root3, anchor='center', bg_color ="#1b1b1b", text=f"{Busqueda2_Telefono}", font=(fuente, 18, "bold"))
...
def Grafico_Login():
    Olvidar_Lugares()
    Texto1.configure(text="Inicio Sesion", font=(fuente, 40))
...
def Grafico_Usuario_Modificar_Datos():
    ...
    def salir():
        ...
        Boton_Salir.place_forget()
        Grafico_Usuario_Cuenta()
    Boton_Usuario_Volver2 = CTkButton(root, image=CTkImage(dark_image=Imagen_Boton_Volver, size=(45, 45)), hover_color="#1b1b1b", text="", width=45, height=45, bg_color="#1b1b1b", fg_color="#1b1b1b", command=salir)
...
def Grafico_Usuario_Casos_Crear():
    ...
    Texto2.configure(text="Tipo Incidente", font=(fuente, 22))
    ...
    Entrada_Caso_TipoIncidente.place(relx=0.33, rely=0.30, anchor="center")
...
def Grafico_Usuario_Casos_Buscar_2():
    ...
    Texto6.configure(text="Incidente:", font=(fuente, 22))
    ...
    Texto14.configure(text=f"{Radicado_Incidente}", font=(fuente, 18, "bold"))
...
def Grafico_Admin_Modificar_Casos():
    global Boton_Admin_Volver2, Entrada_Caso_Departamento, Entrada_Caso_Departamento_Check, Entrada_Caso_Descripcion2, Entrada_Caso_Descripcion2_Check, Entrada_Caso_Fecha, Entrada_Caso_Fecha_Check, Entrada_Caso_Afectados, Entrada_Caso_Afectados_Check, Entrada_Caso_Estado, Entrada_Caso_Estado_Check, Entrada_Caso_Incidente, Entrada_Caso_Incidente_Check, Entrada_Caso_Estado, Entrada_Caso_Estado_Check, Entrada_Caso_Tipo, Entrada_Caso_Tipo_Check
    Olvidar_Lugares()
    ...
    Texto5.configure(text="Incidente:", font=(fuente, 20))
    ...
    Entrada_Caso_Tipo = CTkOptionMenu(root, dropdown_text_color="black", text_color="#404040", values=["Caso", "Ticket"], corner_radius=32, fg_color="#ABABAB", button_color="#404040", dropdown_fg_color="#ABABAB", bg_color="#1b1b1b", font=(fuente, 15), width=160, height=17, anchor="w", variable=Tipo_Var, state="disabled")
    ...
    Incidente_Var = tk.StringVar(value=Incidente_Convertido)
    Entrada_Caso_Incidente = CTkOptionMenu(root, dropdown_text_color="black", text_color="#404040", values=["Desplazamiento", "Hurto", "Expropiacion", "Predios-Despojados"], corner_radius=32, fg_color="#ABABAB", button_color="#404040", dropdown_fg_color="#ABABAB", bg_color="#1b1b1b", font=(fuente, 15), width=160, height=17, anchor="w", variable=Incidente_Var, state="disabled")
    Entrada_Caso_Incidente.place(relx=0.25, rely=0.38, anchor="center")
    Incidente_Check_Var = tk.BooleanVar(value=False)
    Entrada_Caso_Incidente_Check = CTkCheckBox(root, text="", variable=Incidente_Check_Var, command=lambda: (Entrada_Caso_Incidente.configure(state="normal" if Incidente_Check_Var.get() else "disabled"), Incidente_Var.set(Incidente_Var.get() if Incidente_Check_Var.get() else Incidente_Convertido), Entrada_Caso_Incidente.configure(text_color="black" if Incidente_Check_Var.get() else "#404040")))
    Entrada_Caso_Incidente_Check.place(relx=0.47, rely=0.38, anchor="center")

    Estado_Var = tk.StringVar(value=Estado_Convertido)
    Entrada_Caso_Estado = CTkOptionMenu(root, dropdown_text_color="black", text_color="#404040", values=["Pendiente", "Activo", "Finalizado"], corner_radius=32, fg_color="#ABABAB", button_color="#404040", dropdown_fg_color="#ABABAB", bg_color="#1b1b1b", font=(fuente, 15), width=160, height=17, anchor="w", variable=Estado_Var, state="disabled")
    Entrada_Caso_Estado.place(relx=0.25, rely=0.51, anchor="center")
    Estado_Check_Var = tk.BooleanVar(value=False)
    Entrada_Caso_Estado_Check = CTkCheckBox(root, text="", variable=Estado_Check_Var, command=lambda: (Entrada_Caso_Estado.configure(state="normal" if Estado_Check_Var.get() else "disabled"), Estado_Var.set(Estado_Var.get() if Estado_Check_Var.get() else Estado_Convertido), Entrada_Caso_Estado.configure(text_color="black" if Estado_Check_Var.get() else "#404040")))
    Entrada_Caso_Estado_Check.place(relx=0.47, rely=0.51, anchor="center")

    Afectados_Var = tk.StringVar(value=Afectados_Original)
    Entrada_Caso_Afectados = CTkOptionMenu(root, dropdown_text_color="black", text_color="#404040", values=["1", "2", "3", "4", "5", "6", "7", "8"], corner_radius=32, fg_color="#ABABAB", button_color="#404040", dropdown_fg_color="#ABABAB", bg_color="#1b1b1b", font=(fuente, 15), width=160, height=17, anchor="w", variable=Afectados_Var, state="disabled")
    Entrada_Caso_Afectados.place(relx=0.75, rely=0.38, anchor="center")
    Afectados_Check_Var = tk.BooleanVar(value=False)
    Entrada_Caso_Afectados_Check = CTkCheckBox(root, text="", variable=Afectados_Check_Var, command=lambda: (Entrada_Caso_Afectados.configure(state="normal" if Afectados_Check_Var.get() else "disabled"), Afectados_Var.set(Afectados_Var.get() if Afectados_Check_Var.get() else Afectados_Original), Entrada_Caso_Afectados.configure(text_color="black" if Afectados_Check_Var.get() else "#404040")))
    Entrada_Caso_Afectados_Check.place(relx=0.97, rely=0.38, anchor="center")

    Fecha_Var = tk.StringVar(value=Fecha_Formateada)
    Entrada_Caso_Fecha = DateEntry(root, textvariable=Fecha_Var, state="disabled", selectmode='day', date_pattern='dd/mm/yyyy', locale='es_ES', background='#009929', foreground='#404040', selectbackground='#5a64ff', selectforeground='black', weekendbackground='#d1d1d1', headersbackground='#049c9c', font=(fuente, 10))
    Entrada_Caso_Fecha.place(relx=0.75, rely=0.25, anchor="center")
    Fecha_Check_Var = tk.BooleanVar(value=False)
    Entrada_Caso_Fecha_Check = CTkCheckBox(root, text="", variable=Fecha_Check_Var, command=lambda: (Entrada_Caso_Fecha.configure(state="normal" if Fecha_Check_Var.get() else "disabled"), Fecha_Var.set(Fecha_Var.get() if Fecha_Check_Var.get() else Fecha_Formateada), Entrada_Caso_Fecha.configure(foreground="black" if Fecha_Check_Var.get() else "#404040")))
    Entrada_Caso_Fecha_Check.place(relx=0.97, rely=0.25, anchor="center")

    Descripcion_Var = tk.StringVar(value=Descripcion_Original)
    Entrada_Caso_Descripcion2 = CTkEntry(root, text_color="#404040", textvariable=Descripcion_Var, state="disabled", font=(fuente, 15), width=160, height=17, fg_color="#ABABAB", bg_color="#1b1b1b", corner_radius=20)
    Entrada_Caso_Descripcion2.place(relx=0.75, rely=0.51, anchor="center")
    Descripcion_Check_Var = tk.BooleanVar(value=False)
    Entrada_Caso_Descripcion2_Check = CTkCheckBox(root, text="", variable=Descripcion_Check_Var, command=lambda: (Entrada_Caso_Descripcion2.configure(state="normal" if Descripcion_Check_Var.get() else "disabled"), Descripcion_Var.set(Descripcion_Var.get() if Descripcion_Check_Var.get() else Descripcion_Original), Entrada_Caso_Descripcion2.configure(text_color="black" if Descripcion_Check_Var.get() else "#404040")))
    ...
    def salir():
        ...
        Boton_Salir.place_forget()
        Grafico_Admin_Casos()
    Boton_Admin_Volver2 = CTkButton(root, image=CTkImage(dark_image=Imagen_Boton_Volver, size=(45, 45)), hover_color="#1b1b1b", text="", width=45, height=45, bg_color="#1b1b1b", fg_color="#1b1b1b", command=salir)
...
def Grafico_Admin_Modificar_Usuarios():
    ...
    Entrada_Usuario_Modificar_TipoIdentificacion = CTkOptionMenu(root, dropdown_text_color="black", text_color="#404040", values=["Cedula Ciudadania","Cedula Extranjeria", "Pasaporte", "Tarjeta de identidad"], corner_radius=32, fg_color="#ABABAB", button_color="#404040", dropdown_fg_color="#ABABAB", bg_color="#1b1b1b", font=(fuente, 12), width=100, anchor="w", variable=TipoIdentificacion, state="disabled")
    ...
    Entrada_Usuario_Modificar_PrimerNombre = CTkEntry(root, text_color="#404040", textvariable=PrimerNombre_Var, state="disabled", font=(fuente, 13), width=130, fg_color="#ABABAB", bg_color ="#1b1b1b", corner_radius=20)
    ...
    Entrada_Usuario_Modificar_SegundoNombre = CTkEntry(root, text_color="#404040", textvariable=SegundoNombre_Var, state="disabled", font=(fuente, 13), width=130, fg_color="#ABABAB", bg_color ="#1b1b1b", corner_radius=20)
    ...
    Entrada_Usuario_Modificar_PrimerApellido = CTkEntry(root, text_color="#404040", textvariable=PrimerApellido_Var, state="disabled", font=(fuente, 13), width=130, fg_color="#ABABAB", bg_color ="#1b1b1b", corner_radius=20)
    ...
    Entrada_Usuario_Modificar_SegundoApellido = CTkEntry(root, text_color="#404040", textvariable=SegundoApellido_Var, state="disabled", font=(fuente, 13), width=130, fg_color="#ABABAB", bg_color ="#1b1b1b", corner_radius=20)
    ...
    Entrada_Usuario_Modificar_NombreUsuario = CTkEntry(root, text_color="#404040", textvariable=NombreUsuario_Var, state="disabled", font=(fuente, 13), width=130, fg_color="#ABABAB", bg_color ="#1b1b1b", corner_radius=20)
    ...
    Entrada_Usuario_Modificar_RolUsuario = CTkOptionMenu(root, dropdown_text_color="black", text_color="#404040", values=["Usuario", "Administrador"], corner_radius=32, fg_color="#ABABAB", button_color="#404040", dropdown_fg_color="#ABABAB", bg_color="#1b1b1b", font=(fuente, 12), width=100, anchor="w", variable=RolUsuario_Var, state="disabled")
    ...
    Entrada_Usuario_Modificar_EstadoUsuario = CTkOptionMenu(root, dropdown_text_color="black", text_color="#404040", values=["Usuario Activo", "Usuario Inactivo"], corner_radius=32, fg_color="#ABABAB", button_color="#404040", dropdown_fg_color="#ABABAB", bg_color="#1b1b1b", font=(fuente, 12), width=100, anchor="w", variable=EstadoUsuario_Var, state="disabled")
    ...
    Entrada_Usuario_Modificar_Departamento_Normal = CTkOptionMenu(root, dropdown_text_color="black", text_color="#404040", values=["Cundinamarca"], corner_radius=32, fg_color="#ABABAB", button_color="#404040", dropdown_fg_color="#ABABAB", bg_color="#1b1b1b", font=(fuente, 12), width=100, anchor="w", variable=Departamento_Normal_Var, state="disabled")
    ...
    Entrada_Usuario_Modificar_Direccion = CTkEntry(root, text_color="#404040", textvariable=Direccion_Var, state="disabled", font=(fuente, 13), width=130, fg_color="#ABABAB", bg_color ="#1b1b1b", corner_radius=20)
    ...
    Entrada_Caso_Nacimiento = DateEntry(root, textvariable=Nacimiento_Var, state="disabled", selectmode='day', date_pattern='dd/mm/yyyy', locale='es_ES', background='#009929', foreground='#404040', selectbackground='#5a64ff', selectforeground='black', weekendbackground='#d1d1d1', headersbackground='#049c9c', font=(fuente, 10))
    Entrada_Caso_Nacimiento.place(relx=0.61, rely=0.65, anchor="center")
    Nacimiento_Check_Var = tk.BooleanVar(value=False)
    Entrada_Caso_Nacimiento_Check = CTkCheckBox(root, height=10, width=10, text="", variable=Nacimiento_Check_Var, command=lambda: (Entrada_Caso_Nacimiento.configure(state="normal" if Nacimiento_Check_Var.get() else "disabled"), Nacimiento_Var.set(Nacimiento_Var.get() if Nacimiento_Check_Var.get() else Nacimiento_Formateada), Entrada_Caso_Nacimiento.configure(foreground="black" if Nacimiento_Check_Var.get() else "#404040")))
    Entrada_Caso_Nacimiento_Check.place(relx=0.61, rely=0.71, anchor="center")

    Telefono_Var = tk.StringVar(value=Telefono_Normal)
    Entrada_Usuario_Modificar_Telefono = CTkEntry(root, text_color="#404040", textvariable=Telefono_Var, state="disabled", font=(fuente, 13), width=130, fg_color="#ABABAB", bg_color ="#1b1b1b", corner_radius=20)
    ...
    Entrada_Usuario_Modificar_Email = CTkEntry(root, text_color="#404040", textvariable=Email_Var, state="disabled", font=(fuente, 13), width=130, fg_color="#ABABAB", bg_color ="#1b1b1b", corner_radius=20)
    ...
    def salir():
        ...
        Boton_Admin_Volver3.place_forget()
        Grafico_Admin_Usuarios()
    Boton_Admin_Volver3 = CTkButton(root, image=CTkImage(dark_image=Imagen_Boton_Volver, size=(45, 45)), hover_color="#1b1b1b", text="", width=45, height=45, bg_color="#1b1b1b", fg_color="#1b1b1b", command=salir)
...
Imagen_Fondo = tk.Label(root, image=imagenL, bg="#1b1b1b")
Imagen_Fondo.place(relx=1.01, rely=0.01, anchor="ne")
#===========BOTONES====================================
Boton_Salir = CTkButton(root, image=CTkImage(dark_image=Imagen_Boton_Salir, size=(45, 45)), hover_color="#1b1b1b", text="", width=45, height=45, bg_color="#1b1b1b", fg_color="#1b1b1b", command=root.quit)
Boton_Volver = CTkButton(root, image=CTkImage(dark_image=Imagen_Boton_Volver, size=(45, 45)), hover_color="#1b1b1b", text="", width=45, height=45, bg_color="#1b1b1b", fg_color="#1b1b1b", command=Grafico_Inicio)
Boton_Usuario_Volver = CTkButton(root, image=CTkImage(dark_image=Imagen_Boton_Volver, size=(45, 45)), hover_color="#1b1b1b", text="", width=45, height=45, bg_color="#1b1b1b", fg_color="#1b1b1b", command=Grafico_Usuario_Inicio)
Boton_Admin_Volver = CTkButton(root, image=CTkImage(dark_image=Imagen_Boton_Volver, size=(45, 45)), hover_color="#1b1b1b", text="", width=45, height=45, bg_color="#1b1b1b", fg_color="#1b1b1b", command=Grafico_Admin_Inicio)
Boton_Admin_Volver_Casos = CTkButton(root, image=CTkImage(dark_image=Imagen_Boton_Volver, size=(45, 45)), hover_color="#1b1b1b", text="", width=45, height=45, bg_color="#1b1b1b", fg_color="#1b1b1b", command=Grafico_Admin_Casos)
Boton_Admin_Volver_Usuarios = CTkButton(root, image=CTkImage(dark_image=Imagen_Boton_Volver, size=(45, 45)), hover_color="#1b1b1b", text="", width=45, height=45, bg_color="#1b1b1b", fg_color="#1b1b1b", command=Grafico_Admin_Usuarios)
Boton_Usuario_Casos_Volver = CTkButton(root, image=CTkImage(dark_image=Imagen_Boton_Volver, size=(45, 45)), hover_color="#1b1b1b", text="", width=45, height=45, bg_color="#1b1b1b", fg_color="#1b1b1b", command=Grafico_Usuario_Casos)
Boton_Usuario_Cuenta_Volver = CTkButton(root, image=CTkImage(dark_image=Imagen_Boton_Volver, size=(45, 45)), hover_color="#1b1b1b", text="", width=45, height=45, bg_color="#1b1b1b", fg_color="#1b1b1b", command=Grafico_Usuario_Cuenta) 
Boton_Whatsapp = CTkButton(root, image=CTkImage(dark_image=Imagen_Boton_Whatsapp, size=(70, 70)), hover_color="#1b1b1b", text="", width=70, height=70, bg_color="#1b1b1b", fg_color="#1b1b1b", command=abrir_whatsapp)
Boton_Inicio_Login = CTkButton(root, width=300, height=70, text_color="black", text="Iniciar Sesion", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Login, size=(30, 40)), command=Grafico_Login)
Boton_Inicio_Recuperar = CTkButton(root, width=255, height=70, text_color="black", text="Recuperar Contraseña", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Modificar_Contraseña, size=(23, 43)), command=Grafico_Recuperar_Contraseña)
Boton_Inicio_Crear_Usuario = CTkButton(root, width=300, height=70, text_color="black", text=" Crear Usuario", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Crear_Usuario, size=(45, 45)), command=Grafico_Crear_Usuario) 
Boton_Funcion_Login = CTkButton(root, width=255, height=50, text_color="black", text="Iniciar Sesion", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Login, size=(33, 45)), command=Funcion_Login)
Boton_Funcion_Crear_Usuario = CTkButton(root, width=255, height=50, text_color="black", text=" Crear Usuario", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Crear_Usuario, size=(45, 45)), command=Funcion_Crear_Usuario) 
Boton_Usuario_Casos = CTkButton(root, width=265, height=70, text_color="black", text="Casos", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Usuario_Casos, size=(40, 40)), command=Grafico_Usuario_Casos)
Boton_Usuario_Cuenta = CTkButton(root, width=255, height=70, text_color="black", text="Ajustes de Cuenta", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Usuario_Cuenta, size=(40, 40)), command=Grafico_Usuario_Cuenta)
Boton_Funcion_Cuenta_Modificar_Contraseña = CTkButton(root, width=255, height=50, text_color="black", text="Modificar Contraseña", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Usuario_Cuenta, size=(35, 35)), command=Funcion_Modificar_Contraseña)
Boton_Usuario_Cuenta_Modificar_Contraseña = CTkButton(root, width=255, height=70, text_color="black", text="Modificar Contraseña", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Modificar_Contraseña, size=(22, 50)), command=Grafico_Usuario_Cuenta_Modificar_contraseña)
Boton_Usuario_Cuenta_Modificar_Datos = CTkButton(root, width=280, height=70, text_color="black", text="Modificar Datos", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Usuario_Cuenta_Modificar_Datos, size=(40, 40)), command=Grafico_Usuario_Modificar_Datos)
Boton_Usuario_Casos_Grafico_Crear = CTkButton(root, width=265, height=70, text_color="black", text="Crear Caso", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Usuario_Crear_Caso, size=(40, 40)), command=Grafico_Usuario_Casos_Crear)
Boton_Funcion_Crear_Caso = CTkButton(root, width=255, height=50, text_color="black", text="Crear Caso", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Crear_Usuario, size=(45, 45)), command=Funcion_Crear_Caso)
Boton_Funcion_Buscar_Caso = CTkButton(root, width=255, height=50, text_color="black", text=" Buscar Caso", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Usuario_Buscar_Caso, size=(45, 45)), command=Funcion_Buscar_Caso)
Boton_Funcion_Buscar_Caso2 = CTkButton(root, width=255, height=50, text_color="black", text=" Buscar Caso", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Usuario_Buscar_Caso, size=(45, 45)), command=Funcion_Buscar_Caso2)
Boton_Funcion_Buscar_Usuario = CTkButton(root, width=255, height=50, text_color="black", text=" Buscar Usuario", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Usuario_Buscar_Caso, size=(45, 45)), command=Funcion_Buscar_Usuario)
Boton_Funcion_Buscar_Usuario2 = CTkButton(root, width=255, height=50, text_color="black", text=" Buscar Usuario", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Usuario_Buscar_Caso, size=(45, 45)), command=Funcion_Cargar_Datos_Usuarios)
Boton_Usuario_Casos_Grafico_Buscar = CTkButton(root, width=265, height=70, text_color="black", text="Buscar Casos", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Usuario_Buscar_Caso, size=(45, 45)), command=Grafico_Usuario_Casos_Buscar)
Boton_Funcion_Modificar_Datos_Usuario = CTkButton(root, width=255, height=50, text_color="black", text="Modificar Datos", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Usuario_Cuenta, size=(35, 35)), command=Funcion_Modificar_Contactos_Usuario)
Boton_Admin_Usuarios = CTkButton(root, width=260, height=70, text_color="black", text="Usuarios", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Modificar_Usuario, size=(35, 40)), command=Grafico_Admin_Usuarios)
Boton_Admin_Casos = CTkButton(root, width=260, height=70, text_color="black", text="Casos", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Usuario_Crear_Caso, size=(40, 40)), command=Grafico_Admin_Casos)
Boton_Admin_Entidades = CTkButton(root, width=260, height=70, text_color="black", text="Entidades", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Admin_Entidad, size=(45, 35)), command=Funcion_Buscar_Entidades)
Boton_Funcion_Eliminar_Caso = CTkButton(root, width=255, height=50, text_color="black", text=" Eliminar Caso", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Usuario_Casos, size=(45, 40)), command=Funcion_Eliminar_Datos_Casos)
Boton_Funcion_Eliminar_Usuario = CTkButton(root, width=255, height=50, text_color="black", text=" Eliminar Usuario", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Usuario_Casos, size=(45, 40)), command=Funcion_Eliminar_Usuario)
Boton_Funcion_Modificar_Datos_Caso = CTkButton(root, width=255, height=50, text_color="black", text="Modificar Datos", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Usuario_Cuenta, size=(35, 35)), command=Funcion_Modificar_Datos_Casos)
Boton_Funcion_Modificar_Datos_Usuario5 = CTkButton(root, width=255, height=50, text_color="black", text="Modificar Usuario", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Usuario_Cuenta, size=(35, 35)), command=Funcion_Modificar_Datos_Usuarios)
Boton_Admin_Grafico_Casos_Buscar = CTkButton(root, width=300, height=70, text_color="black", text="Buscar Caso", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Usuario_Buscar_Caso, size=(45, 45)), command=Grafico_Admin_Casos_Buscar)
Boton_Admin_Grafico_Casos_Crear = CTkButton(root, width=300, height=70, text_color="black", text="Crear Caso", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Crear_Usuario, size=(45, 45)), command=Grafico_Admin_Casos_Crear)
Boton_Admin_Grafico_Casos_Modificar = CTkButton(root, width=300, height=70, text_color="black", text="Modificar Caso", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Usuario_Cuenta, size=(45, 45)), command=Grafico_Admin_Modificar_Casos_Buscar)
Boton_Admin_Grafico_Casos_Eliminar = CTkButton(root, width=300, height=70, text_color="black", text="Eliminar Caso", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Eliminar, size=(37, 37)), command=Grafico_Admin_Eliminar_Casos)
Boton_Funcion_Crear_Usuario2 = CTkButton(root, width=255, height=50, text_color="black", text=" Crear Usuario", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Crear_Usuario, size=(45, 45)), command=Funcion_Crear_Usuario2) 
Boton_Funcion_Recuperar_Contraseña = CTkButton(root, width=255, height=50, text_color="black", text="Enviar UUID", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Modificar_Contraseña, size=(20, 30)), command=Funcion_Recuperar_Contraseña) 
Boton_Funcion_Recuperar_Contraseña2 = CTkButton(root, width=255, height=50, text_color="black", text="Recuperar Usuario", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Modificar_Contraseña, size=(20, 30)), command=Funcion_Validar_Token_Cambiar)  
Boton_Admin_Grafico_Usuarios_Buscar = CTkButton(root, width=300, height=70, text_color="black", text="Buscar Usuario", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Usuario_Buscar_Caso, size=(45, 45)), command=Grafico_Usuario_Admin_Buscar)
Boton_Admin_Grafico_Usuarios_Crear = CTkButton(root, width=300, height=70, text_color="black", text="Crear Usuario", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Crear_Usuario, size=(45, 45)), command=Grafico_Admin_Usuarios_Crear)
Boton_Admin_Grafico_Usuarios_Modificar = CTkButton(root, width=300, height=70, text_color="black", text="Modificar Usuario", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Usuario_Cuenta, size=(45, 45)), command=Grafico_Admin_Modificar_Usuarios_Buscar)
Boton_Admin_Grafico_Usuarios_Eliminar = CTkButton(root, width=300, height=70, text_color="black", text="Eliminar Usuario", font=(fuente, 25), fg_color="#009929", hover_color="#006414", compound="right", corner_radius=120, border_color="#00420D", border_width=5, bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Eliminar, size=(37, 37)), command=Grafico_Admin_Eliminar_Usuarios)
Boton_Pagina = CTkButton(root, image=CTkImage(dark_image=Imagen_Boton_Usuario_Datos, size=(120, 120)), hover_color="#1b1b1b", text="", width=120, height=120, bg_color="#1b1b1b", fg_color="#1b1b1b", command=Funcion_Pagina)

Imagen_Grafico_Usuario_Datos_Documento = CTkLabel(root, text="", bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Usuario_Datos_Documento, size=(42, 35)))
Imagen_Grafico_Usuario_Datos_Correo = CTkLabel(root, text="", bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Usuario_Datos_Correo, size=(42, 35)))
Imagen_Grafico_Usuario_Datos_Telefono = CTkLabel(root, text="", bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Usuario_Datos_Telefono, size=(35, 35)))
Imagen_Grafico_Usuario_Datos_Direccion = CTkLabel(root, text="", bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Usuario_Datos_Direccion, size=(40, 40)))
Imagen_Grafico_Login_Usuario = CTkLabel(root, text="", bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Modificar_Usuario, size=(33, 40)))
Imagen_Grafico_Login_Contraseña = CTkLabel(root, text="", bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Modificar_Contraseña, size=(25, 50)))
Imagen_Grafico_Modificar_Contraseña = CTkLabel(root, text="", bg_color="#1b1b1b", image=CTkImage(dark_image=Imagen_Boton_Login, size=(30, 40)))

Contenedor = CTkFrame(root, corner_radius=10)
Entrada_UUID = CTkEntry(root, font=(fuente, 13), fg_color="#ABABAB", text_color="black", bg_color ="#1b1b1b", corner_radius=20)
Entrada_Usuario_Identificacion = CTkEntry(root, font=(fuente, 13), fg_color="#ABABAB", text_color="black", bg_color ="#1b1b1b", corner_radius=20)
Entrada_Caso_TipoIncidente = CTkOptionMenu(root, dropdown_text_color="black", values=["Desplazamiento","Hurto", "Expropiacion", "Predios-Despojados"], text_color="black", corner_radius=32, fg_color="#ABABAB", button_color="#404040", dropdown_fg_color="#ABABAB", bg_color ="#1b1b1b", font=(fuente, 13), width=120, anchor="w")
Entrada_Caso_Personas = CTkOptionMenu(root, dropdown_text_color="black", values=["1","2","3","4","5","6","7","8"], corner_radius=32, fg_color="#ABABAB", text_color="black", button_color="#404040", dropdown_fg_color="#ABABAB", bg_color ="#1b1b1b", font=(fuente, 13), width=120, height=18, anchor="w")
Entrada_Caso_Fecha = DateEntry(root,selectmode='day', year=2009, month=11, day=11, locale='es_ES', date_pattern='dd/mm/yyyy', background='#009929', foreground='black', selectbackground="#5aff70", selectforeground='black', weekendbackground='#d1d1d1', headersbackground="#5d9c04", font=(fuente, 10))
Entrada_Caso_Descripcion = CTkTextbox(root, font=(fuente, 20), width=400, height=150, fg_color="#ABABAB", border_color="#3d3c3c", border_width=2, text_color="black", corner_radius=15, bg_color="#1b1b1b")
Entrada_Caso_Direccion = CTkEntry(root, font=(fuente, 13), fg_color="#ABABAB", text_color="black", bg_color ="#1b1b1b", corner_radius=20)
Entrada_Caso_Radicado = CTkEntry(root, font=(fuente, 13), fg_color="#ABABAB", text_color="black", bg_color ="#1b1b1b", corner_radius=20)
Entrada_Contraseña_Nueva = CTkEntry(root, show="*", font=(fuente, 13), fg_color="#ABABAB", text_color="black", bg_color ="#1b1b1b", corner_radius=20)
Entrada_Usuario_Nueva = CTkEntry(root, font=(fuente, 13), fg_color="#ABABAB", text_color="black", bg_color ="#1b1b1b", corner_radius=20)
Entrada_Primer_Nombre = CTkEntry(root, font=(fuente, 13), fg_color="#ABABAB", text_color="black", bg_color ="#1b1b1b", corner_radius=20)
Entrada_Segundo_Nombre = CTkEntry(root, font=(fuente, 13), fg_color="#ABABAB", text_color="black", bg_color ="#1b1b1b", corner_radius=20)
Entrada_Primer_Apellido = CTkEntry(root, font=(fuente, 13), fg_color="#ABABAB", text_color="black", bg_color ="#1b1b1b", corner_radius=20)
Entrada_Segundo_Apellido = CTkEntry(root, font=(fuente, 13), fg_color="#ABABAB", text_color="black", bg_color ="#1b1b1b", corner_radius=20)
Entrada_Tipo_Documento = CTkOptionMenu(root, dropdown_text_color="black", values=["Cedula Ciudadania","Cedula Extranjeria", "Pasaporte", "Tarjeta de identidad"], corner_radius=32, text_color="black", fg_color="#ABABAB", button_color="#404040", dropdown_fg_color="#ABABAB", bg_color ="#1b1b1b", font=(fuente, 13), width=120, anchor="w")
Entrada_Documento = CTkEntry(root, font=(fuente, 13), fg_color="#ABABAB", text_color="black", bg_color ="#1b1b1b", corner_radius=20)
Entrada_Fecha_Nacimento = DateEntry(root,selectmode='day', year=2009, month=11, day=11, locale='es_ES', date_pattern='dd/mm/yyyy', background='#009929', foreground='black', selectbackground='#5aff70', selectforeground='black', weekendbackground='#d1d1d1', headersbackground='#5d9c04', font=(fuente, 10))
Entrada_Email = CTkEntry(root, font=(fuente, 13), fg_color="#ABABAB", text_color="black", bg_color ="#1b1b1b", corner_radius=20)
Entrada_Departamento = CTkOptionMenu(root, dropdown_text_color="black", values=["Cundinamarca"], corner_radius=32, fg_color="#ABABAB", button_color="#404040", text_color="black", dropdown_fg_color="#ABABAB", bg_color ="#1b1b1b", font=(fuente, 15), width=120, anchor="w")
Entrada_Ciudad = CTkEntry(root, font=(fuente, 13), fg_color="#ABABAB", text_color="black", bg_color ="#1b1b1b", corner_radius=20)
Entrada_Localidad = CTkEntry(root, font=(fuente, 13), fg_color="#ABABAB", text_color="black", bg_color ="#1b1b1b", corner_radius=20)
Entrada_Barrio = CTkEntry(root, font=(fuente, 13), fg_color="#ABABAB", text_color="black", bg_color ="#1b1b1b", corner_radius=20)
Entrada_Direccion = CTkEntry(root, font=(fuente, 13), fg_color="#ABABAB", text_color="black", bg_color ="#1b1b1b", corner_radius=20)
Entrada_Numero = CTkEntry(root, font=(fuente, 13), fg_color="#ABABAB", text_color="black", bg_color ="#1b1b1b", corner_radius=20)
Entrada_Usuario = CTkEntry(root, font=(fuente, 13), fg_color="#ABABAB", text_color="black", bg_color ="#1b1b1b", corner_radius=20)
Entrada_Contraseña = CTkEntry(root, show="*", font=(fuente, 13), fg_color="#ABABAB", text_color="black", bg_color ="#1b1b1b", corner_radius=20)

Texto1 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto2 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto3 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto4 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto5 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto6 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto7 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto8 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto9 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto10 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto11 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto12 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto13 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto14 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto15 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto16 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto17 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto18 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto19 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto20 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto21 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto22 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto23 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto24 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto25 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto26 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto27 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto28 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto29 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto30 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto31 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto32 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto33 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto34 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto35 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto36 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto37 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")
Texto38 = CTkLabel(root, text="", font=(fuente, 20), anchor='center', bg_color ="#1b1b1b", text_color="black")

Texto_Admin = CTkLabel(root, text="Modo Administrador", font=(fuente, 16), anchor='center', bg_color ="#1b1b1b", text_color="black")
#==============================================================================================
...
#==============================================================================================
Boton_Whatsapp.place(relx=0.99, rely=0.99, anchor="se")
Grafico_Inicio()
root.mainloop()