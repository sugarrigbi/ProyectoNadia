from App.Models.Account_Model import Dispositivos as Tabla_Dispositivos, Dispositivos_Auditoria as Tabla_Auditoria, Usuario as Tabla_Usuario, Permiso as Tabla_Permiso, Persona as Tabla_Persona
...
class Account_Service:
    ...
    def Read_Devices(User_ID):
        ...
        if "dispositivo_ver" in Nombres:
            Dispositivos = Tabla_Dispositivos.query.filter(Tabla_Dispositivos.Usuario_ID == Usuario_Validar.ID, Tabla_Dispositivos.Estado_Dispositivo_ID != 3).order_by(Tabla_Dispositivos.ID).all()
            return Dispositivos
        else:
            return "Auth"
    @staticmethod
